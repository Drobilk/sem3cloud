...
TOKEN = "6942185931:AAG1_8tc-51r4ynmwl81rGeM_A-dUMZvKBw"
...
def tattoo_choice(update: Update, context: CallbackContext) -> int:
    ...
    if user_response == 'да':
        update.message.reply_text('Отлично! Есть пожелания к эскизу? Вариант того, что хочется набить? '
                                  'Разработка эскиза бесплатная. Опишите, включая размер, стиль. Фото можно будет прикрепить на следующем этапе')
...
def tattoo_preferences(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text('Отлично! Теперь пришлите фото для разработки эскиза.')
    return TATTOO_PHOTO
...
def tattoo_photo(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text('Спасибо за фото! Теперь укажите свой номер телефона для связи:')
    return CONTACT_INFO
...
def contact_info(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text('Теперь выберите удобную дату для сеанса:', reply_markup=get_available_dates_markup())
...
def get_available_dates():
    ...
    cursor.execute('SELECT date FROM appointment')
...
file_handler = FileHandler('bot_logs.log')
